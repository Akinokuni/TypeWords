...
import { DICT_LIST, LIB_JS_URL } from '@/core/config/env.ts'
...
import WordFailedTable from '~/components/import/WordFailedTable.vue'
import type { FailedWordRow } from '~/components/import/WordFailedTable.vue'

type ImportStep = 1 | 2 | 3
type ImportType = 'word' | 'article'
type ImportMode = 'official' | 'custom' | 'blank'
...
// 官方词库（本地静态词库）单词 -> 释义 的缓存
let officialWordMap: Map<string, Word> | null = null
...
const officialCount = computed(() => importSummary.value?.officialCount ?? 0)
...
const manualWords = computed(() =>
  textInput.value
    .split('\n')
    .map(item => item.trim().replaceAll(',', '').replaceAll('，', '').replaceAll('"', ''))
    .filter(Boolean)
)
...
function loadImportSummary(targetId: string): ImportResultSummary | null {
  try {
    ...
    const pendingFailedWords =
      summary.pendingFailedWords?.map(row => ({ ...row, editing: false })) ??
      (type === 'word' && summary.failedItems?.length
        ? summary.failedItems.map(word => ({ id: nanoid(6), word, checked: true, editing: false }))
        : undefined)
    const successCount = summary.successCount ?? 0
    const importMode = summary.importMode ?? (type === 'word' ? 'official' : undefined)
    return {
      successCount,
      officialCount: summary.officialCount ?? (importMode === 'official' ? successCount : 0),
...
function restoreFromRoute() {
  ...
  if (step.value === 3) {
    ...
    if (summary) {
      ...
      step.value = 2
      router.replace({
        query: { ...route.query, type: importType.value, step: '2', targetId: targetId || undefined },
      })
      return
    }
  }
...
async function persistTarget() {
  ...
  const dict = getDefaultDict({
    ...target,
    id: `custom-${importType.value}-${nanoid(8)}`,
    custom: true,
    type: isWord.value ? DictType.word : DictType.article,
  })
...
function completeImport(dict: Dict, summary: ImportResultSummary) {
  ...
  step.value = 3
  router.replace({
    query: { ...route.query, type: importType.value, step: '3', targetId: String(saved.id) },
  })
}
...
async function downloadTemplate(kind: 'word' | 'custom-word' | 'article') {
  try {
    const XLSX = await loadJsLib('XLSX', LIB_JS_URL.XLSX)
    const rows: Record<string, string>[] = []
    if (kind === 'word') {
      rows.push({ 单词: 'cancel' }, { 单词: 'explosive' })
    } else if (kind === 'custom-word') {
      rows.push({
        单词: 'apple',
        '音标①': 'ˈæp(ə)l',
        '音标②': 'ˈæpəl',
    ...
    XLSX.writeFile(wb, `import-${kind}-template.xlsx`)
  } catch (error: any) {
    Toast.error(error?.message || '模板下载失败，请稍后重试')
  }
}

// ============ 官方词库查词 ============

// 加载本地官方词库（/list/word.json 里列出的所有 word 词库）并建立 小写单词 -> 释义 映射
async function loadOfficialWordMap(): Promise<Map<string, Word>> {
  if (officialWordMap) return officialWordMap
  const map = new Map<string, Word>()
  try {
    const list = await fetch(resourceWrap(DICT_LIST.WORD.ALL)).then(r => r.json())
    const dicts = Array.isArray(list) ? list : []
    for (const d of dicts) {
      const url = d?.url
      if (!url) continue
      try {
        const language = d?.language ?? 'en'
        const words = await fetch(resourceWrap(`/dicts/${language}/word/${url}`)).then(r => r.json())
        if (Array.isArray(words)) {
          for (const w of words) {
            if (w?.word) map.set(String(w.word).toLowerCase(), w as Word)
          }
        }
      } catch {
        // 单个词库加载失败则跳过
...
function createFailedWordRows(words: string[]): FailedWordRow[] {
  return words.map(word => ({ id: nanoid(6), word, checked: true, editing: false }))
}
...
// 官方词库查词后合并：找到的单词带释义，找不到的进入“未收录”列表
async function mergeWordsFromList(
  ...
  const map = await loadOfficialWordMap()
  const foundItems: Word[] = []
  const missing: string[] = []
  for (const word of filtered) {
    const def = map.get(word.toLowerCase())
    if (def) {
      foundItems.push(getDefaultWord({ ...def, id: nanoid(6), custom: false, word: def.word ?? word }))
    } else {
      missing.push(word)
    }
  }
  ...
  foundItems.forEach(w => next.words.push(w))
  ...
  const summary: ImportResultSummary = {
    ...
    skippedCount,
    failedItems: missing,
    pendingFailedWords: createFailedWordRows(missing),
    type: 'word',
  }
  return { next, summary }
}
...
async function retryFailedImport() {
  ...
  try {
    ...
    rows.forEach(row => {
      ...
      const def = map.get(word.toLowerCase())
      if (!def) return
      const targetWord = def.word ?? word
      if (existsSet.has(targetWord)) {
        succeededRowIds.add(row.id)
        return
      }
      dict.words.push(getDefaultWord({ ...def, id: nanoid(6), custom: false, word: targetWord }))
      existsSet.add(targetWord)
      succeededRowIds.add(row.id)
      added++
    })
...
async function importSelectedWordFile() {
...
function submitWordImport() {
  ...
  if (hasOfficialFile.value) return importSelectedWordFile()
...
function finishArticleImport(
  ...
  completeImport(next, { successCount, skippedCount, failedItems, type: 'article' })
}

async function importSelectedArticleFile() {
  if (!selectedFile.value) return Toast.warning('请先选择要上传的文件')
        ...
        <section v-if="step === 1" class="workflow-section">
          ...
          <template v-if="pendingDict">
            ...
            <div class="flex gap-4 flex-wrap">
              <Book :is-add="false" :item="pendingDict" :quantifier="isWord ? '词' : '篇'" :show-progress="false" :selected="true" />
            </div>
          </template>
        ...
        <section v-else-if="step === 2" class="workflow-section">
          ...
          <div class="input-split">
            ...
            <div class="method-panel primary-method">
              ...
              >
                ...
                <div class="flex gap-3 flex-col mb-4">
                  <div v-if="isWord">
                    <a href="#" @click.prevent="downloadTemplate('word')">下载单词列表 xlsx 模板</a>
                  </div>
                  <div>
                    <a href="#" @click.prevent="downloadTemplate(isWord ? 'word' : 'article')">
                      下载 {{ contentLabel }} xlsx 模板
                    </a>
                  </div>
                </div>
              ...
              <template v-if="isWord">
                <div class="custom-import-panel" :class="{ 'import-block--disabled': customUploadDisabled }">
                  ...
                  <div class="my-2 color-gray text-sm">下载模板，填写翻译、音标、例句等字段后上传</div>
                  <div class="flex gap-3 flex-col mb-4">
                    <div>
                      <a href="#" @click.prevent="downloadTemplate('custom-word')">下载自定义单词 xlsx 模板</a>
                    </div>
                  </div>
                  <div class="flex items-center gap-3 flex-wrap">
                    <UploadButton
            ...
            <div class="method-panel" v-else>
              ...
              <p>选择左侧的 .json 或 .xlsx 文件后提交即可批量导入文章。</p>
            </div>
          </div>
          ...
          <div class="actions-row step2-actions">
            ...
            <BaseButton
              ...
              @click="importSelectedArticleFile"
            >
              提交
            </BaseButton>
          </div>
          ...
          <div class="status-strip mb-4">
            ...
            <div class="ml-10">
              ...
              <li v-if="hasPendingFailedWords">
                ...
                <ul class="mt-1 pl-4">
                  <li>可在下方表格修改后再次导入</li>
                  <li>可点击“下载失败列表”按钮，填写翻译后，返回上一步通过「导入自定义单词」上传</li>
                </ul>
              </li>
              <li v-else-if="failedCount && !isWord">{{ failedCount }} 个失败（原因：缺少标题或正文必填字段）</li>
            </div>
          </div>
